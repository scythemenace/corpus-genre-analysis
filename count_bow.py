...
open_txt_files_in_directory(
    "./text_data/crime_mystery/hound_of_baskerville", crime_docs
)
...
def count_words(documents, bog, **kwargs):
    ...
    for doc in documents:
        tokens = normalize_text_module.normalize_text(
            doc, bog, **kwargs
        )  # kwargs just helps me to avoid writing default values for each option here
...
P_w_romance = {
    word: calculate_probability(word, romance_bog, total_words["romance"], vocab_size)
    for word in vocab
}
P_w_crime = {
    word: calculate_probability(word, crime_bog, total_words["crime"], vocab_size)
    for word in vocab
}

LLR_romance = {
    word: math.log(P_w_romance[word]) - math.log(P_w_crime[word]) for word in vocab
}
LLR_crime = {
    word: math.log(P_w_crime[word]) - math.log(P_w_romance[word]) for word in vocab
}
...
def topic_distribution(lda_model, romance_docs, crime_docs, dictionary):
    ...
    romance_avg_dist = {
        topic: prob / len(romance_docs) for topic, prob in romance_topic_sum.items()
    }
    crime_avg_dist = {
        topic: prob / len(crime_docs) for topic, prob in crime_topic_sum.items()
    }

    return romance_avg_dist, crime_avg_dist


romance_avg_dist, crime_avg_dist = topic_distribution(
    lda_model, romance_docs_tk, crime_docs_tk, dictionary
)
